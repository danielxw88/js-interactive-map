window.onload = () => {
  ...
  window.addToFavorites = (name, address) => {
    if (!favoriteSpots.some((spot) => spot.name === name)) {
      favoriteSpots.push({ name, address });
      displayFavorites();
    } else {
  ...
  window.removeFromFavorites = (name) => {
    const index = favoriteSpots.findIndex((spot) => spot.name === name);
    if (index !== -1) {
  ...
  const displayFavorites = () => {
    ...
    sidebar.innerHTML = "<h3>Your Favorite Spots</h3>";
    favoriteSpots.forEach((spot) => {
      sidebar.innerHTML += `<div>
        <p><strong>${spot.name}</strong></p>
        <p>${spot.address}</p>
        <button onclick="removeFromFavorites('${spot.name}')">Remove</button>
      </div>`;
    });
  };
};
